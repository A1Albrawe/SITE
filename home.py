...
<style>
    ...
    .cyber-matrix-rain { position: fixed; top: -100px; left: 0; width: 100%; height: 100%; background: linear-gradient(to bottom, transparent, rgba(0, 255, 102, 0.12) 50%, transparent); z-index: 1; pointer-events: none; animation: matrixRainFall 8s linear infinite; opacity: 0.3; }
...
<body>
        ...
        <div class="responsive-profile-wrapper">
            ...
            <div class="profile-content-zone">
                <div class="details-sub-box">
                    <span class="meta-item">⚡ <span class="meta-label">[SYS_INFO]:</span> بناء وتطوير تطبيقات الويب الكاملة، وتصميم وتعديل اسكريبتات البايثون الحصينة.</span>
    ...
    <script>
        // 📡 خوارزمية الإقلاع المعزول الفوري: ضرب الـ API فوراً بشكل حر ومستقل دون انتظار جلب الموقع
        document.addEventListener("DOMContentLoaded", () => {
            let assignedName = localStorage.getItem('cyber_assigned_username');
            if (!assignedName) {
                assignedName = "Hacker_" + Math.floor(1000 + Math.random() * 9000);
                localStorage.setItem('cyber_assigned_username', assignedName);
            }
            
            // إطلاق شفرة تسجيل الزيارة فوراً لمنع تجميد الأرقام
            const payload = { username: assignedName, location: "القاهرة - مصر" };
            fetch('/api/log_visit', {
                method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload)
            });

            // جدولة وضخ الثواني الحية بانتظام صارم كل 4 ثوانٍ
            setInterval(() => {
                fetch('/api/update_duration', {
                    method: 'POST', headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ username: assignedName, game: 'browsing', durationIncrement: 4 })
                });
            }, 4000);

            // فرز الموقع بشكل فرعي صيانة لـ CORS
            fetch('https://ipapi.co').then(res => res.json()).then(locData => {
                if(locData.city) {
                    fetch('/api/log_visit', {
                        method: 'POST', headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({ username: assignedName, location: locData.city + " - " + locData.country_name })
                    });
                }
            }).catch(()=>{});
        });
        ...
        if (window.innerWidth > 850) {
            ...
            document.querySelector(".responsive-profile-wrapper").addEventListener("mouseleave", () => { avatarBox.style.transform = "perspective(500px) scale(1) rotateX(0deg) rotateY(0deg)"; });
        }
